...
## 🔴 写文件前强制自检（4/21 新增，对应 Iron Law #4）

每次收藏任何内容，在写入 collections/ 文件**之前**必须通过以下自检。缺一项不允许写文件。
...
- [ ] 去重检查 ✔
- [ ] 正文提取 ✔
- [ ] Schema.org 提取（获取不到静默跳过） ✔
- [ ] 插图提取 + 下载（🔴 强制） ✔
- [ ] 主题关键词提取 ✔
- [ ] 写 collections/ 文件（含插图引用） ✔
- [ ] 同步 Obsidian KaiVault（含插图复制） ✔
- [ ] Daily Note 联动 ✔
```

### 视频类（YouTube/B站/小红书/抖音）
```
写文件前自检（缺一不可）：
- [ ] 去重检查 ✔
- [ ] 元数据提取 ✔
...
**规则**：
- 每次收藏完成后，在回复用户前内心过一遍 checklist，确认所有项已完成
- 某一步确实无法执行（如 browser 不可用无法提取插图）→ 在收藏文件中标注 `incomplete: true` + 缺失项说明，不能静默跳过
- 老板明确说“只要摘要”“不用图片”等 → 可跳过对应项，但其他项仍必须完成
